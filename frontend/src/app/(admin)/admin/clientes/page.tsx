    ...
    <div className="max-w-7xl mx-auto px-6 py-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
          ...
          <p className="text-2xl font-bold text-green-400">{clients.filter(c => c.onboardingComplete).length}</p>
        </div>
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
          <p className="text-gray-500 text-xs">Cadastro Pendente</p>
          <p className="text-2xl font-bold text-yellow-400">{clients.filter(c => !c.onboardingComplete).length}</p>
        </div>
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
          <p className="text-gray-500 text-xs">Com Contratos</p>
          <p className="text-2xl font-bold text-indigo-400">{clients.filter(c => c._count.contracts > 0).length}</p>
        </div>
      </div>
      ...
      <div className="mb-4">
        <div className="relative max-w-md">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-gray-900 border border-gray-800 rounded-xl text-white text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none placeholder:text-gray-600"
          />
        </div>
      </div>
              ...
              <tbody className="divide-y divide-gray-800">
                {filtered.map(client => (
                  <tr key={client.id} className="hover:bg-gray-800/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm text-gray-300">{client.company || '-'}</td>
                    <td className="px-6 py-4 text-sm text-gray-400 font-mono">{client.cnpj || '-'}</td>
                    <td className="px-6 py-4">
                      {client.onboardingComplete ? (
                        ...
                        <span className="text-xs px-2 py-0.5 rounded-full text-yellow-400 bg-yellow-900/30">
                          Pendente
                        </span>
                      )}
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-400">{client._count.documents}</td>
                    <td className="px-6 py-4 text-sm text-gray-400">{client._count.contracts}</td>
